...
const EvaluasiAkhir = () => {
  ...
  const [timeLeft, setTimeLeft] = useState(2400); // 40 menit
    ...
    const fetchQuestions = async () => {
      try {
        ...
        if (filteredQuestions.length < 20) {
          setError(
            `Hanya ${filteredQuestions.length} soal tersedia. Diperlukan 20 soal untuk Evaluasi Akhir.`
          );
          setQuestions(filteredQuestions || []);
        } else {
          setQuestions(filteredQuestions.slice(0, 20)); // Ambil hanya 20 soal
        }
      } catch (err) {
  ...
  const handleNextOrFinish = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      if (!areAllQuestionsAnswered()) {
        alert(
          "Masih ada soal yang belum dijawab. Silakan lengkapi semua jawaban sebelum menyelesaikan."
        );
        return;
      }
      handleFinish();
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      try {
        ...
        const message =
          scoreError.response?.data?.message || scoreError.message;
        throw new Error(`Gagal menyimpan skor: ${message}`);
      }
    } catch (err) {
