...
const KelolaSoal = () => {
  ...
  const handleAddOrUpdateQuestion = async () => {
    try {
      ...
      Swal.fire({
        title: 'Berhasil',
        text: response.data.message || (isEditMode ? 'Soal berhasil diperbarui.' : 'Soal berhasil ditambahkan.'),
        icon: 'success',
        confirmButtonText: 'OK',
        confirmButtonColor: '#255F38',
      });
      ...
      if (err.response?.status === 413) {
        ...
        Swal.fire({
          title: 'Gagal',
          text: err.response?.data?.message || `Gagal menyimpan soal: ${err.message}`,
          icon: 'error',
          confirmButtonText: 'OK',
          confirmButtonColor: '#255F38',
        });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire({
            title: 'Gagal',
            text: err.response?.data?.message || 'Gagal menghapus soal. Silakan coba lagi.',
            icon: 'error',
            confirmButtonText: 'OK',
            confirmButtonColor: '#255F38',
          });
              ...
              {currentQuestions.map((question, index) => (
                ...
                >
                  ...
                  <td className="p-2 border-b border-gray-200 flex space-x-2 justify-center">
                    <button
                      onClick={() => openEditModal(question)}
                      className="px-2 py-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-200 text-sm cursor-pointer"
                    >
                      Edit
                    </button>
                    <button
                ...
                {previewImage && (
                  <div className="mt-2 flex flex-col items-center">
                    ...
                    <button
                      ...
                      className="mt-2 px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors duration-200 text-sm cursor-pointer"
                    >
                      Hapus Gambar
                    </button>
                  </div>
