...
const QuizPencarianData = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/students/questions?quizNumber=${selectedQuiz}`, {
  ...
  const handleQuizChange = (e) => {
    ...
    setAnswers({}); // Reset jawaban saat kuis berubah
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isNaN(score) || score < 0 || score > 100) {
        throw new Error("Skor yang dihitung tidak valid");
      }
      const token = localStorage.getItem('token');
      ...
      scorePayload[`kuis${selectedQuiz}`] = score;
      ...
      navigate("/feedback1", { state: { answers, correctCount, score, questions } });
    } catch (error) {
      console.error("Error in handleSubmit:", error);
      alert("Terjadi kesalahan saat menyelesaikan kuis. Silakan coba lagi.");
    }
  ...
  return (
    <Layout>
      ...
      <div className="mb-4">
        <label className="mr-2">Pilih Kuis:</label>
        <select value={selectedQuiz} onChange={handleQuizChange} className="border p-2 rounded">
      ...
      <div>
        <p className="text-gray-700 text-sm md:text-base text-justify leading-relaxed px-4 mb-8">
          Selamat datang di kuis tentang fungsi VLOOKUP, HLOOKUP, INDEX, MATCH, dan CHOOSE! Kuis ini berisi soal pilihan ganda untuk melatih pemahamanmu dalam mencari data. Pilih jawaban yang benar untuk setiap soal. Skor minimal 70 diperlukan untuk lanjut ke materi berikutnya. Selamat mengerjakan!
        </p>
      </div>

      {questions.length === 0 ? (
        <p className="text-center text-gray-500">Belum ada soal untuk kuis ini.</p>
      ) : (
        questions.map((q) => (
          <div key={q.id} className="p-4 mb-4 bg-white rounded shadow-md">
            <h3 className="text-md font-semibold text-green-700">Soal {q.id}</h3>
            <p className="text-gray-700 mt-2">{q.question_text}</p>
            {q.image_url && <img src={q.image_url} alt={`Gambar untuk soal ${q.id}`} className="mt-2 max-w-xs" />}
