...
const QuizPengelolaanData = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/quizzes/questions?quizNumber=4', {
          headers: { Authorization: `Bearer ${token}` }
        });
        const filteredQuestions = response.data.filter(q => q.quiz_number === 4);
  ...
  useEffect(() => {
    if (timeLeft > 0) {
      ...
      handleFinish(); // Auto-finish when time is up
    }
  }, [timeLeft]);
