...
const kkmModel = {
  async getKKMSettings() {
    try {
      const [rows] = await pool.query('SELECT quiz_number, kkm FROM kkm_settings ORDER BY quiz_number');
      if (rows.length === 0) {
        // Return default KKM values if table is empty
        return [
  ...
  async updateKKMSettings(settings) {
    ...
    try {
      ...
      for (const setting of settings) {
        ...
        if (![1, 2, 3, 4].includes(quiz_number)) {
