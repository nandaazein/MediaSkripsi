...
const kkmController = {
  ...
  async updateKKMSettings(req, res) {
    try {
      const { kkmValues } = req.body; // Expect {1: 70, 2: 70, 3: 70, 4: 70}
      if (!kkmValues || typeof kkmValues !== 'object') {
        return res.status(400).json({ message: 'KKM values harus berupa objek' });
      }
      const settings = Object.keys(kkmValues).map(quiz_number => ({
        quiz_number: parseInt(quiz_number),
        kkm: parseInt(kkmValues[quiz_number])
      }));
      if (settings.length !== 4 || settings.some(s => isNaN(s.kkm) || s.kkm < 0 || s.kkm > 100 || ![1, 2, 3, 4].includes(s.quiz_number))) {
        return res.status(400).json({ message: 'Harus menyertakan KKM valid untuk 4 kuis' });
      }
  ...
  async getKKMByQuizNumber(req, res) {
    try {
      ...
      if (isNaN(parsedQuizNumber) || ![1, 2, 3, 4].includes(parsedQuizNumber)) {
